...
(function (global) {
  ...
  function renderDrillMiniCards(row, containerEl) {
    ...
    var p = row.platforms || {};
    var specs = [
      { k: "Web", click: nt().nCount ? nt().nCount((p.desktop || {}).click) : 0, impr: nt().nCount ? nt().nCount((p.desktop || {}).impression) : 0 },
      { k: "MWeb", click: nt().nCount ? nt().nCount((p.mobileweb || {}).click) : 0, impr: nt().nCount ? nt().nCount((p.mobileweb || {}).impression) : 0 },
      { k: "Android", click: nt().nCount ? nt().nCount((p.android || {}).click) : 0, impr: nt().nCount ? nt().nCount((p.android || {}).impression) : 0 },
    ...
    if (sbProfiles.web || sbProfiles.mweb) {
      ...
      var rows6 = keys.map(function (key) {
        ...
        ["web", "mweb"].forEach(function (pf) {
          var sb = sbProfiles[pf];
          if (!sb || !sb.buckets) return;
          for (var i = 0; i < sb.buckets.length; i++) {
            if (sb.buckets[i].key === key) {
              if (pf === "web") w = Number(sb.buckets[i].sessions || 0);
              else m = Number(sb.buckets[i].sessions || 0);
            }
          }
        });
        if (!w && !m) return "";
  ...
  global.NTExtras = {
    ...
    renderDrillMiniCards: renderDrillMiniCards,
  };
