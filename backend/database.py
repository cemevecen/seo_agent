...
def ensure_indexes() -> None:
    ...
    with engine.connect() as conn:
        ...
        def _ensure_notification_workspace_col(name: str, sqlite_ddl: str, pg_ddl: str) -> None:
            try:
                if _IS_SQLITE:
                    rc = conn.execute(
                        _txt(
                            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='notification_analytics_workspace'"
                        )
                    )
                    if not rc.fetchone():
                        return
                    cols = {
                        row[1]
                        for row in conn.execute(
                            _txt("PRAGMA table_info(notification_analytics_workspace)")
                        ).fetchall()
                    }
                    if name not in cols:
                        conn.execute(
                            _txt(f"ALTER TABLE notification_analytics_workspace ADD COLUMN {name} {sqlite_ddl}")
                        )
                else:
                    try:
                        conn.execute(
                            _txt(
                                f"ALTER TABLE notification_analytics_workspace ADD COLUMN IF NOT EXISTS {name} {pg_ddl}"
                            )
                        )
                    except Exception:  # noqa: BLE001
                        conn.execute(
                            _txt(f"ALTER TABLE notification_analytics_workspace ADD COLUMN {name} {pg_ddl}")
                        )
            except Exception as exc:  # noqa: BLE001
                LOGGER.debug("notification_analytics_workspace ADD COLUMN %s atlandı: %s", name, exc)

        _ensure_notification_workspace_col("last_file_upload_at", "DATETIME", "TIMESTAMP")

        def _ensure_home_drive_upload_col(name: str, sqlite_ddl: str, pg_ddl: str) -> None:
            try:
                if _IS_SQLITE:
                    rc = conn.execute(
                        _txt(
                            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='home_drive_uploads'"
                        )
                    )
                    if not rc.fetchone():
                        return
                    cols = {
                        row[1]
                        for row in conn.execute(_txt("PRAGMA table_info(home_drive_uploads)")).fetchall()
                    }
                    if name not in cols:
                        conn.execute(_txt(f"ALTER TABLE home_drive_uploads ADD COLUMN {name} {sqlite_ddl}"))
                else:
                    try:
                        conn.execute(
                            _txt(
                                f"ALTER TABLE home_drive_uploads ADD COLUMN IF NOT EXISTS {name} {pg_ddl}"
                            )
                        )
                    except Exception:  # noqa: BLE001
                        conn.execute(_txt(f"ALTER TABLE home_drive_uploads ADD COLUMN {name} {pg_ddl}"))
            except Exception as exc:  # noqa: BLE001
                LOGGER.debug("home_drive_uploads ADD COLUMN %s atlandı: %s", name, exc)

        _ensure_home_drive_upload_col(
            "container_key", "VARCHAR(64) NOT NULL DEFAULT ''", "VARCHAR(64) NOT NULL DEFAULT ''"
        )
        _ensure_home_drive_upload_col(
            "container_label",
