...
from backend.api.sheet_schedule import router as sheet_schedule_router
...
def admin_run_market_sheets_sync_now():
    """Eski tablo senkronu kapalı; piyasa serileri tarama ingest ile gelir."""
    return {
        "status": "disabled",
        "message": "Piyasa serileri tarama ile güncellenir (00:05 TR).",
    }
...
app.include_router(sheet_schedule_router, prefix="/api")
...
def _template_sheet_menu_visible(request: Request | None) -> bool:
    if request is None:
        return False
    return bool(getattr(request.state, "sheet_menu_visible", False))
...
async def ip_allowlist_middleware(request: Request, call_next):
    ...
    public_prefixes = (
        ...
        "/api/play-console/coverage",
        "/api/play-analytics/ga4-trend-ingest",
        "/api/asc-console/ingest",
        "/api/asc-console/coverage",
        "/api/firebase-console/ingest",
        "/api/firebase-console/coverage",
        ...
        "/api/sinemalar-moderation/purge",
        "/api/sinemalar-moderation/coverage",
        "/api/sinemalar-moderation/gaps",
        "/api/sinemalar-moderation/meta",
    ...
    if panel_authed:
        ...
        request.state.sheet_menu_visible = resolve_sheet_menu_visible(
            member_email=member.email if member else None,
        )
        if member is not None:
        ...
        if is_sheet_page_path(path):
            em = member.email if member else None
            if not is_sheet_page_allowed_email(em or ""):
                if path.startswith("/api/"):
                    return JSONResponse(
                        status_code=403,
        ...
        out.append(
            {
                ...
                "top20_drops": drop_payload.get("top20_drops") or [],
                "top20_rises": drop_payload.get("top20_rises") or [],
                "as_of_label": drop_payload.get("as_of_label"),
                "as_of_iso": drop_payload.get("as_of_iso"),
                "scope_label": drop_payload.get("scope_label"),
                "top20_scope_label": drop_payload.get("top20_scope_label"),
