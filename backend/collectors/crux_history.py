...
def _histogram_bin_category(metric_key: str, start: float) -> str:
    """CrUX histogram bin başlangıcına göre good / ni / poor (Google CWV eşikleri)."""
    if metric_key == "largest_contentful_paint":
        if start < 2500:
            return "good"
        if start < 4000:
            return "ni"
...
def histogram_status_timeseries(record: dict | None) -> dict:
    """CrUX History kaydından good / needs-improvement / poor yükleme oranları (%), zaman serisi.

    Search Console’daki “URL sayısı” grafikleri değil; CrUX’un gerçek kullanıcı yükleme dağılımıdır.
    """
    ...
    for period in periods:
        if not isinstance(period, dict):
            labels.append("")
            continue
        ld = period.get("lastDate") or {}
        y, m, d = ld.get("year"), ld.get("month"), ld.get("day")
        labels.append(f"{y:04d}-{m:02d}-{d:02d}" if y and m and d else "")

    metrics_root = record.get("metrics") or {}
    ...
    for metric_key in HISTOGRAM_METRICS:
        if metric_key not in METRIC_LABELS:
            continue
        metric_payload = metrics_root.get(metric_key) or {}
        bins = metric_payload.get("histogramTimeseries") or []
        if not isinstance(bins, list) or not bins:
            continue

        if period_count == 0:
            first = bins[0] if bins else {}
            dens = first.get("densities") if isinstance(first, dict) else None
            if isinstance(dens, list) and dens:
                period_count = len(dens)
                labels = [str(i + 1) for i in range(period_count)]

        good = [0.0] * period_count
        ni = [0.0] * period_count
        poor = [0.0] * period_count

        for bin_obj in bins:
            if not isinstance(bin_obj, dict):
                continue
            start = _safe_number(bin_obj.get("start"))
            if start is None:
                continue
            cat = _histogram_bin_category(metric_key, float(start))
            dens = bin_obj.get("densities") or []
            if not isinstance(dens, list):
                continue
            for t in range(min(period_count, len(dens))):
                v = _safe_number(dens[t])
                if v is None:
                    continue
                p = v * 100.0
                if cat == "good":
    ...
    return {"labels": labels[:period_count] if period_count else labels, "metrics": out_metrics}
