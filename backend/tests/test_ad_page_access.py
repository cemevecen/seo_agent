# -*- coding: utf-8 -*-
"""/ad-virgul erisim allowlist testleri."""
...
def test_ad_page_paths():
    ...
    assert is_ad_page_path("/api/virgul-analytics/summary")
    assert is_ad_page_path("/api/virgul-analytics/ingest")
...
def test_ad_menu_visible_and_denied():
    ...
    assert resolve_ad_menu_visible(member_email="other@nokta.com") is True
    assert resolve_ad_menu_visible(member_email="gozdeunaldi@nokta.com") is False
    assert resolve_ad_menu_visible(member_email="berendemirci@gmail.com") is False
    assert resolve_ad_menu_visible(member_email=None) is False
    assert member_denied_ad_access("other@nokta.com") is False
    assert member_denied_ad_access("gozdeunaldi@nokta.com") is True
